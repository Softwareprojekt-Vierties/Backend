        ...
        for (let provider of serviceProvider)
        {
            if(provider["userid"]>0)
            {
                ...
                if(provider["type"]=="artist")
                {
                    ...
                    let service = await CreateQueries.createMail(ownerid, provider['userid'], 'service', eventid)
...
async function updateMail(userid, id, gelesen, angenommen = null) {
    try {
        ...
        if (mail.rows.length > 0 && mail.rows[0]['empfaenger'] === userid) {
            ...
            if (mail.rows[0]['anfrage'] === 'location') eventMailResponse("location",angenommen,mail.rows[0]["sender"],mail.rows[0]['eventid'])
